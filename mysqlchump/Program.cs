...
class Program
{
	...
	private static RootCommand CreateRootCommand()
	{
		...
		var importCommand = new Command("import", "Imports data to a database")
		{
			...
			insertIgnoreOption, csvImportColumnsOption, csvUseHeadersOption, csvFixInvalidMysqlOption, inputFileArgument, importNoCreationOption
		};
		...
		importCommand.SetHandler(async context =>
		{
			...
			if (Directory.Exists(inputFile))
			{
				...
				while (fileQueue.Count > 0)
				{
					...
					var exitCode = await ImportMainAsync(connectionString, inputFormat, nextFile, importTable,
						(csvUseHeaders || string.IsNullOrWhiteSpace(csvColumns)) ? null : csvColumns.Split(',').Select(x => x.Trim()).ToArray(),
						csvFixInvalidMysql, csvUseHeaders, insertIgnore, noCreate);
			...
			context.ExitCode = await ImportMainAsync(connectionString, inputFormat, inputFile, importTable,
				(csvUseHeaders || string.IsNullOrWhiteSpace(csvColumns)) ? null : csvColumns.Split(',').Select(x => x.Trim()).ToArray(),
				csvFixInvalidMysql, csvUseHeaders, insertIgnore, noCreate);
		});
	...
	static async Task<int> ImportMainAsync(string connectionString, InputFormatEnum inputFormat, string inputLocation, string table, string[] csvColumns, bool csvFixInvalidMysql, bool csvUseHeaders, bool insertIgnore, bool noCreate)
	{
		...
		try
		{
			...
			else if (inputFormat == InputFormatEnum.json)
			{
				...
				await importer.ImportAsync(currentStream, insertIgnore, createConnection, noCreate);
			}
