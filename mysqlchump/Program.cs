...
class Program
{
	...
	private static RootCommand CreateRootCommand()
	{
		...
		var importCommand = new Command("import", "Imports data to a database")
		{
			...
			insertIgnoreOption, csvImportColumnsOption, csvUseHeadersOption, csvFixInvalidMysqlOption, inputFileArgument
		};
		...
		importCommand.SetHandler(async context =>
		{
			...
			context.ExitCode = await ImportMainAsync(connectionString, inputFormat, result.GetValueForArgument(inputFileArgument), importTable,
				...
				csvFixInvalidMysql, csvUseHeaders, insertIgnore);
		});
	...
	static async Task<int> ImportMainAsync(string connectionString, InputFormatEnum inputFormat, string inputLocation, string table, string[] csvColumns, bool csvFixInvalidMysql, bool csvUseHeaders, bool insertIgnore)
	{
