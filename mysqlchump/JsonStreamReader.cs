...
public class JsonStreamReader
{
	...
	public JsonStreamReader(Stream stream, byte[] buffer)
	{
		_stream = stream;
		_buffer = buffer;
    ...
    public int Advance()
    {
        ...
        int read = _stream.Read(_buffer.AsMemory(leftoverLength).Span);
        _readCount = read + leftoverLength;

		return read;
    }

    public async ValueTask<int> AdvanceAsync()
    {
        // Save off existing text
        int leftoverLength = FlipBuffer();
    ...
    public JsonTokenType? ReadToken()
    {
    ...
    public async ValueTask<JsonTokenType?> ReadTokenAsync()
    {
		if (ReadNext(out var tokenType))
			return tokenType;

		UpdateState(GetReader());

		await AdvanceAsync();

		if (ReadNext(out tokenType))
			return tokenType;
